...
/**
 * Sanitiza Celular: Elimina espacios y prefijos (+591 o 591)
 * para validar solo los 8 dígitos.
 */
  ...
  ci: z
    ...
    .pipe(
      z
        ...
        .max(CI_MAX_LENGTH, { message: `Formato no válido` })
        .superRefine((val, ctx) => {
  ...
  celular: z
    ...
    .pipe(
      z
        ...
        .regex(
          REGEX_CELULAR_BOLIVIA,
          `El campo Celular (debe empezar con 6 o 7, ser numérico y tener ${CELULAR_MIN_LENGTH} dígitos).`
        )
    ),

  area_nivel_ids: z
    .array(z.number())
    .min(1, { message: 'Debe asignar al menos un área-nivel.' }),
});
