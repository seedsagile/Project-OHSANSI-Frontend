...
export function useSeleccionAreaNivel({
  ...
  useEffect(() => {
    ...
    const todosLosIdsDisponibles = new Set<number>(
      areasDisponibles.flatMap((area) =>
        area.niveles.map((nivel) => nivel.id_area_nivel)
      )
    );
    ...
    if (gestionPasadaSeleccionadaAnio) {
      ...
      const asignacionesPasadas = detalles?.asignaciones_evaluador ?? [];

      const idsGestionPasada = new Set(
        asignacionesPasadas.map((a) => a.id_area_nivel)
      );
      setAreasFromPastGestion(idsGestionPasada);

      const idsCargables = Array.from(idsGestionPasada).filter(
        (id) =>
          todosLosIdsDisponibles.has(id) &&
          !preAsignadas.has(id)
      );
