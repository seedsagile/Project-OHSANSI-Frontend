...
export function FormularioAsignarEvaluador({ register, errors }: Props) {
  ...
  const handleEmailInput = (e: React.FormEvent<HTMLInputElement>) => {
    ...
    // Detectar si intentan ingresar espacios (incluso múltiples)
    const hasSpaces = /\s/.test(originalValue);
    
    // Detectar si SOLO hay espacios (campo vacío visualmente pero con espacios)
    const onlySpaces = originalValue.trim() === '' && originalValue.length > 0;
  ...
  const handleEmailPaste = (e: React.ClipboardEvent<HTMLInputElement>) => {
    e.preventDefault();
    const pastedText = e.clipboardData.getData('text');
    const hasSpaces = /\s/.test(pastedText);
    
    // Detectar si SOLO hay espacios en el texto pegado
    const onlySpaces = pastedText.trim() === '' && pastedText.length > 0;
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div>
          ...
          {!errors.apellido && limitMessages.apellido && (
            <p className="text-acento-600 text-sm mt-1">{limitMessages.apellido}</p>
          )}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
          ...
          {!errors.password && limitMessages.password && (
            <p className="text-acento-600 text-sm mt-1">{limitMessages.password}</p>
          )}
        </div>
        ...
        <div>
          ...
          {!errors.password_confirmation && limitMessages.password_confirmation && (
            <p className="text-acento-600 text-sm mt-1">{limitMessages.password_confirmation}</p>
          )}
        </div>
      </div>
      ...
      <div>
        ...
        {!errors.codigo_evaluador && limitMessages.codigo_evaluador && (
          <p className="text-acento-600 text-sm mt-1">{limitMessages.codigo_evaluador}</p>
        )}
