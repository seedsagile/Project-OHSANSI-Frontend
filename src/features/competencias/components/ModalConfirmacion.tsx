// src/features/competencias/components/ModalConfirmacion.tsx
...
export function ModalConfirmacion({
  ...
  return (
    <div
      className="fixed bg-black/70 inset-0 flex items-center justify-center z-50 p-4"
      onClick={type === 'success' ? undefined : onClose}
      role="dialog"
      aria-modal="true"
    >
      <div
        className="bg-white rounded-xl shadow-2xl w-full max-w-md p-8 text-center border-2 border-gray-300"
        onClick={(e) => e.stopPropagation()}
      >
        {iconMap[type]}
        <h2 className="text-2xl font-bold text-gray-800 mt-4">{title}</h2>
        <div className="text-gray-600 mt-2 text-md">{children}</div>
