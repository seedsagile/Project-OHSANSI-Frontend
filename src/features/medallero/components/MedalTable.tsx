...
export const MedalTable = ({ medalData, onUpdate, isParametrized }: MedalTableProps) => {
  const handleIncrement = (index: number, field: keyof Omit<MedalData, 'id_area_nivel' | 'nombre_nivel' | 'ya_parametrizado'>) => {
    if (isParametrized) return;
  ...
  const handleDecrement = (index: number, field: keyof Omit<MedalData, 'id_area_nivel' | 'nombre_nivel' | 'ya_parametrizado'>) => {
    if (isParametrized) return;
              ...
              {(['oro', 'plata', 'bronce', 'menciones'] as const).map((field) => (
                <td key={field} className="px-6 py-4 text-center">
                  <div className="flex items-center justify-center gap-1">
                    <button
                      type="button"
                      onClick={() => handleDecrement(index, field)}
                    ...
                    <button
                      type="button"
                      onClick={() => handleIncrement(index, field)}
                      disabled={isParametrized}
                      className="w-6 h-6 flex items-center justify-center bg-gray-200 hover:bg-gray-300 rounded disabled:opacity-50 disabled:cursor-not-allowed"
                    >
                      <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
