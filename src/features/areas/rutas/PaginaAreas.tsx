...
export function PaginaAreas() {
  ...
  const cargarAreas = async () => {
    try {
      ...
      setError(
        "Error al cargar las áreas. Verifique que el servidor esté funcionando."
      );
    }
  };
    ...
    try {
      ...
      if (error instanceof Error) {
        if (error.message.toLowerCase().includes("existe")) {
          mensajeError =
            "Ya existe un área con este nombre. Por favor ingrese un nombre diferente.";
        } else if (
    ...
    <div className="min-h-screen p-6">
      ...
      <div className="max-w-7xl mx-auto relative">
        ...
        <div className="grid grid-cols-2 gap-8">
                  ...
                  {areas.length === 0 ? (
                    ...
                    areas.map((area, index) => (
                      <tr
                        ...
                        className={`cursor-pointer border-b border-blue-300 hover:bg-blue-100 transition-colors ${
          ...
          <div className="pl-8">
            <MockupNiveles areaSeleccionada={areaSeleccionada} />
          </div>
        </div>
      </div>
