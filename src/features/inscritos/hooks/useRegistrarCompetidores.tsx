...
import { headerMapping, normalizarEncabezado, type ProcesamientoCSVResult } from '../utils/csvProcessor';
...
export function useImportarCompetidores() {
    ...
    useEffect(() => {
        ...
        workerRef.current.onmessage = (e: MessageEvent<{ type: string; payload: ProcesamientoCSVResult | string }>) => {
            const { type, payload } = e.data;
            setIsParsing(false);

            if (type === 'SUCCESS' && typeof payload !== 'string') {
                if (payload.errorGlobal) {
                    setModalState({ isOpen: true, type: 'error', title: 'Error Crítico en Archivo', message: payload.errorGlobal });
                    setNombreArchivo(null);
                    setFilas([]);
                    setColumnasDinamicas([]);
