...
export const procesarYValidarCSV = (
    ...
    .object({
      ...
      colegio_institucion: z
        ...
        .superRefine((val, ctx) => {
          if (val.length === 0)
            ctx.addIssue({ code: z.ZodIssueCode.custom, message: 'Este campo es obligatorio.' });
          else if (!/^[a-zA-Z0-9\s.\-áéíóúÁÉÍÓÚñÑüÜ]+$/.test(val))
            ctx.addIssue({
              code: z.ZodIssueCode.custom,
      ...
      celular_tutor: z
        .string()
        .optional()
        .transform((val) => (val ? sanitizePhoneNumber(val.replace(/\s/g, '')) : val)),
    ...
    .superRefine((data, ctx) => {
      ...
      if (!areaEncontrada) {
        const sugerencia = getSuggestion(
          data.area,
          areasConNiveles.map((a) => a.nombre)
        );
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
      ...
      if (!nivelValidoEnArea) {
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
          message: 'Este nivel no está asignado a esta área.',
          path: ['nivel'],
        });
      }
    });
  ...
  const filasProcesadas: FilaProcesada[] = filasPreProcesadas.map((fila) => {
    ...
    if (signatureDuplicates && signatureDuplicates.length > 1) {
      const otraFila =
        signatureDuplicates.find((n) => n !== numeroDeFila) || signatureDuplicates[0];
