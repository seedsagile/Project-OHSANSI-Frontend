  ...
  const filaSchema = z.object({
    ...
    ci: z
      ...
      .superRefine((val, ctx) => {
        if (val.length === 0)
          ctx.addIssue({ code: z.ZodIssueCode.custom, message: 'Este campo es obligatorio.' });
        else if (!/^[0-9]+(-?[A-Z])?$/.test(val))
          ctx.addIssue({
            code: z.ZodIssueCode.custom,
            message: 'Formato inválido. Ej: 7962927, 7962927A, 7962927-A.',
          });
      }),
    ...
    genero: z
      ...
      .superRefine((val, ctx) => {
        if (val.length === 0)
          ...
          ctx.addIssue({
            code: z.ZodIssueCode.custom,
            message: 'Debe ser una sola letra: "M" o "F".',
          });
      }),
    ...
    celular_estudiante: z
      .string()
      .transform((val) => val.trim())
      .superRefine((val, ctx) => {
        if (val.length === 0)
          ctx.addIssue({ code: z.ZodIssueCode.custom, message: 'Este campo es obligatorio.' });
        else if (!/^[0-9]+$/.test(sanitizePhoneNumber(val)))
    ...
    area: z
      ...
      .superRefine((val, ctx) => {
        if (val.length === 0)
          ...
          !areasValidas.some((a) => normalizeForComparison(a) === normalizeForComparison(val))
        )
          ctx.addIssue({
            code: z.ZodIssueCode.custom,
            message:
              `El área no existe. ${getSuggestion(val, areasValidas as string[]) || ''}`.trim(),
          });
      })
      .transform(toTitleCase),
    ...
    nivel: z
      ...
      .superRefine((val, ctx) => {
        if (val.length === 0)
          ...
          !nivelesValidos.some((n) => normalizeForComparison(n) === normalizeForComparison(val))
        )
          ctx.addIssue({
            code: z.ZodIssueCode.custom,
            message:
              `El nivel no existe. ${getSuggestion(val, nivelesValidos as string[]) || ''}`.trim(),
          });
      })
      .transform(toTitleCase),
    ...
    fecha_nacimiento: z
      .string()
      .optional()
      .refine((val) => !val || !isNaN(Date.parse(val)), { message: 'Formato de fecha inválido.' }),
    ...
    capacidad_del_grupo: z
      .string()
      .regex(/^[0-9]+$/, { message: 'Capacidad debe ser un número.' })
      .optional(),
  });
  ...
  datosCompletos.forEach(({ datosLimpios, rawData }, i) => {
    ...
    if (rowSignatures.has(rowSignature)) {
      filasProcesadas.push({
        ...
        rawData,
        esValida: false,
        errores: { general: 'Fila duplicada en el archivo.' },
        numeroDeFila,
      });
      return;
    }
    ...
    if (!validationResult.success) {
      ...
      if (ciSet.has(dataValidada.ci)) {
        filasProcesadas.push({
          ...
          rawData,
          esValida: false,
          errores: { ci: 'CI duplicado en el archivo.' },
          numeroDeFila,
        });
      } else {
