...
export const requiredCSVKeys: (keyof CompetidorCSV)[] = ['ci', 'nombres', 'apellidos', 'genero', 'departamento', 'colegio_institucion', 'celular_estudiante', 'email', 'area', 'nivel'];
...
export const procesarYValidarCSV = (
    ...
    const filaSchema = z.object({
        ...
        departamento: z.string().trim().min(1, 'Departamento (DEP.) es obligatorio.').refine(val => DEPARTAMENTOS_VALIDOS.some(d => d.toLowerCase() === val.toLowerCase()), { message: 'Departamento no válido.' }).transform(toTitleCase),
        ...
        area: z.string().trim().min(1, 'Área es obligatoria.').superRefine((val, ctx) => {
            if (!areasValidas.find(a => a.toLowerCase() === val.toLowerCase())) ctx.addIssue({ code: z.ZodIssueCode.custom, message: `El Área "${val}" no es válida. ${getSuggestion(val, areasValidas) || ''}`.trim() });
        }),
        nivel: z.string().trim().min(1, 'Nivel es obligatorio.').superRefine((val, ctx) => {
            if (!nivelesValidos.find(n => n.toLowerCase() === val.toLowerCase())) ctx.addIssue({ code: z.ZodIssueCode.custom, message: `El Nivel "${val}" no es válido. ${getSuggestion(val, nivelesValidos) || ''}`.trim() });
        }),
    ...
    if (cleanText.charCodeAt(0) === 0xFEFF) { cleanText = cleanText.substring(1); } // Remover BOM
    ...
    if (encabezadosFaltantes.length > 0) {
        ...
        const errorMessage = `Faltan las siguientes columnas obligatorias: ${encabezadosFaltantes.join(', ')}`;
    ...
    datosComoObjetos.forEach((fila, i) => {
        ...
        if (rowSignatures.has(rowSignature)) {
            filasProcesadas.push({ datos: fila, esValida: false, errores: { general: 'Fila duplicada.' }, numeroDeFila });
            return;
        }
        rowSignatures.add(rowSignature);
        const validationResult = filaSchema.safeParse(fila);
        if (!validationResult.success) {
            const errores = validationResult.error.issues.reduce((acc, issue) => { acc[issue.path.join('.')] = issue.message; return acc; }, {} as { [key: string]: string });
