...
export function PaginaImportarCompetidores() {
  ...
  const hayErroresDeFila = useMemo(
    () => datos.length > 0 && datos.some((d) => !d.esValida),
    [datos]
  );
          ...
          <Alert
            ...
            message={
              <div>
                ...
                <p className="mt-1">
                  Para evitar problemas con acentos y caracteres especiales (ej: Pérez, Potosí),
                  asegúrese de que su archivo esté guardado en formato{' '}
          ...
          <section className="my-8">
            {isLoadingData ? (
              <div className="flex flex-col items-center justify-center h-48 rounded-lg border-2 border-dashed border-neutro-300 bg-neutro-50 text-center">
                ...
                <p className="text-sm text-neutro-500">Por favor, espere un momento.</p>
              </div>
            ) : (
              <DropzoneArea
